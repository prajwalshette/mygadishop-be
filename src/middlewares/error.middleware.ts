...
export const ErrorMiddleware = (error: HttpException | Error, req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (
      ...
      logger.error(
        `[PRISMA ERROR] [${req.method}] ${req.path} ${JSON.stringify({
          statusCode: status,
          message,
          field,
    ...
    // 3. Handle DatabaseException
      ...
      logger.error(
        `[DATABASE ERROR] [${req.method}] ${req.path} ${JSON.stringify({
          statusCode: status,
          message,
          field,
        })}`,
    ...
    // 4. Handle all other custom HTTP exceptions
      ...
      logger.error(
        `[${errorType}] [${req.method}] ${req.path} ${JSON.stringify({
          statusCode: status,
          message,
        })}`,
      );
    ...
    // 5. Handle generic/unknown errors
      ...
      status = 500;
      message = process.env.NODE_ENV === 'development' ? error.message || 'An unexpected error occurred' : 'An unexpected error occurred';

      logger.error(
        `[UNHANDLED ERROR] [${req.method}] ${req.path} ${JSON.stringify({
          message: error.message,
          stack: error.stack,
